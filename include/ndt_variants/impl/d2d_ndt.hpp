...
D2DNDT<PointSource, PointTarget>::D2DNDT (float resolution, double step_size, double outlier_ratio)
  : target_cells_()
...
D2DNDT<PointSource, PointTarget>::computeTransformation (PointCloudSource &output, const Eigen::Matrix4f &guess)
{
  ...
  while (!converged_)
  {
    ...
    delta /= delta_norm;
    delta_norm = computeStepLengthMT (delta, delta_norm, step_size_, transformation_epsilon_ / 2, score, score_gradient, hessian);
    delta *= delta_norm;
...
D2DNDT<PointSource, PointTarget>::computeDerivatives(Eigen::Matrix<double, 6, 1> &score_gradient,
                                                     Eigen::Matrix<double, 6, 6> &hessian,
                                                     bool compute_hessian)
{
  ...
  for (size_t i = 0; i < trans_covs_.size(); i++)
  {
    ...
    for (const auto& cell: neighborhood)
    {
      ...
      score += updateDerivatives (score_gradient, hessian, x_diff, c_inv, compute_hessian);
    }
  }
  return score;
}
...
template<typename PointSource, typename PointTarget> void
D2DNDT<PointSource, PointTarget>::computePointDerivatives (const Eigen::Vector3d &x, bool compute_hessian)
{
...
template<typename PointSource, typename PointTarget> double
D2DNDT<PointSource, PointTarget>::updateDerivatives (Eigen::Matrix<double, 6, 1> &score_gradient,
                                                     Eigen::Matrix<double, 6, 6> &hessian,
...
template<typename PointSource, typename PointTarget> void
D2DNDT<PointSource, PointTarget>::updateHessian (Eigen::Matrix<double, 6, 6> &hessian, const Eigen::Vector3d &x_trans, const Eigen::Matrix3d &c_inv) const
{
  // e^(-d_2/2 * (x_k - mu_k)^T Sigma_k^-1 (x_k - mu_k)) Equation 6.9 [Magnusson 2009]
...
D2DNDT<PointSource, PointTarget>::updateIntervalMT (double &a_l, double &f_l, double &g_l,
...
template<typename PointSource, typename PointTarget> double
D2DNDT<PointSource, PointTarget>::trialValueSelectionMT (double a_l, double f_l, double g_l,
                                                                               double a_u, double f_u, double g_u,
                                                                               double a_t, double f_t, double g_t) const
{
...
template<typename PointSource, typename PointTarget> double
D2DNDT<PointSource, PointTarget>::computeStepLengthMT (Eigen::Matrix<double, 6, 1> &step_dir, double step_init, double step_max,
