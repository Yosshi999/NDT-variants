...
int main(int argc, char* argv[])
{
  ...
  Eigen::Affine3f affine = Eigen::Translation3f(0.1, 0, 0) * Eigen::AngleAxisf(0.1*M_PI, Eigen::Vector3f::UnitX());
  ...
  boost::function< void(const PC&, const std::vector<int>&, const PC&, const std::vector<int>) >
    callback =
    [&regvis](const PC& _src, const std::vector<int> &indsrc, const PC& _tgt, const std::vector<int> &indtgt) {
      PPC src(new PC), tgt(new PC);
      pcl::copyPointCloud(_src, *src);
      pcl::copyPointCloud(_tgt, *tgt);
      regvis->removeAllPointClouds();
      pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> red(src, 255, 0, 0);
      regvis->addPointCloud<pcl::PointXYZ>(src, red, "source");
      pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> green(tgt, 0, 255, 0);
